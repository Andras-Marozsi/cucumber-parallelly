...
class Helper {
   ...
   */
  static getScenarios(tagsToGet, weightingTags, fPaths) {
    var
      allScenarios = [],
      sortedScenarios = [],
      tagExpression = '',
    ...
    if (typeof tagsToGet == 'array') {
      tagsToGet.map(function (tag, index) {
        if (index != 0) {
          tagExpression += ' and '
        }
        if (tag.indexOf('~') == -1 && tag.indexOf(',') == -1) {
          ...
          tagExpression += "not " + tag;
        } else if (tag.indexOf('~') != -1 && tag.indexOf(',') != -1) {
          tagExpression += "not (" + tag.replace(/,/g, ' or ') + ")";
        } else if (tag.indexOf('~') == -1 && tag.indexOf(',') != -1) {
          tagExpression += "(" + tag.replace(/,/g, ' or ') + ")";
        }
      })
    } else {
      tagExpression = tagsToGet;
    }
    ...
    function getFeaturesByTagExpression() {
      return getTestCasesFromFilesystem({
        ...
        allScenarios.map(function (scenario) {
          sortedScenarios.push(scenario.uri + ':' + scenario.pickle.locations[0].line);
        });
