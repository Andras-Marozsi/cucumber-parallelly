...
class Helper {
  ...
  static hadUndefinedStep(pathToReport) {
    try {
      ...
      console.error("Couldn't process report file: " + pathToReport);
      return false;
    }
  };
  ...
  static getScenarios(tagsToGet, weightingTags, featuresPaths) {
    ...
    features.map(function (feature) {
      feature.getScenarios().map(function (scenario) {
    ...
    allScenarios.map(function (scenario) {
      sortedScenarios.push(pathHelper.normalize(scenario.getUri()) + ':' + scenario.getLine());
    });
