 ...
 * @constructor
 */
var ScenariosOnWorkerPool = function ScenariosOnWorkerPool(config) {
  var maxWorkerCount = config.threads === undefined ? 5 : config.threads;
  var maxRetryCount = config.retries === undefined ? 0 : config.retries;
  var hideOutput = config.silent === undefined ? false : config.silent;
  var exec = require('child_process').exec;
  var REPORT_FILE = config.reportPath || './reports/report.json';
  var TEMP_REPORT_PATH = config.tempReportPath || './reports/tmp/';
  var RETRY_STR = '_RETRY_';
  var activeWorkers = 0;
  var allTests = 0;
  var failedTests = 0;
  var passedTests = 0;
  var tagsToGet = { tags: config.tags || [] };
  var arrayOfScenarioPaths = [];
  var testResults = {};
  var startTime = void 0;
  var helper = require('./helper.js');
  var path = require('path');
  var reportCreator = require('./report_creator.js');
  var command = "node " + config.cucumberPath + " " + config.cucumberOpts;
  ...
  function processScenarios() {
    while (activeWorkers < maxWorkerCount && arrayOfScenarioPaths.length > 0) {
      var scenario = arrayOfScenarioPaths.shift();
      var tempReportFile = (TEMP_REPORT_PATH + scenario.replace(/.*\/features\//, '').replace(/:/g, '_').replace('.', '_') + ".json").replace(/\\/g, '/');
  ...
  function result() {
    ...
    var failedAfterRetry = 0;
    var executionDuration = new Date() - startTime;
    var min = Math.floor(executionDuration / 1000 / 60);
    var sec = Math.floor((executionDuration / 1000).toFixed(2) - min * 60);
